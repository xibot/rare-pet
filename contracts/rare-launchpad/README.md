...
**Draft, locally tested and exercised against mainnet with read-only simulation; not deployed or audited.** No deployment or launch transaction has been broadcast. At block **72725075**, constructor `eth_call` produced the exact runtime with its configured immutables. Overriding only that prospective router runtime and its WETH allowlist slot, both the real Genesis #2 owner → canonical RF `execute` → router → real Doppler modules route and the direct self route completed successfully. Self mode also verified the creator-equals-treasury fee merge. All existing RF and Doppler contracts used actual pinned mainnet state. No simulated state persisted. Local tests cover persisted Brain, cooldown and failure behavior.

The conventional Forge fork remains unavailable through this public RPC (older state was pruned and a fresh Forge request received HTTP 403). The state-override rehearsal uses ordinary supported `eth_call` requests and does not bypass that restriction. Evidence is generated in `state-override-review.json`; the unsigned deployment review binds its SHA-256 and creation-bytecode hash.
...
Initial quote list, verified against Robinhood’s issuer directory and Bankr’s Doppler quote registry:
...
The local suite contains 26 tests, including 256-case fuzz tests for invalid curve weights and invalid creation receipts. It checks exact cooldown boundaries, timestamp zero, transferred NFTs, token ID zero, impersonation, metadata/quote/fee constraints, full-pool policy, the confirmed 85/10/5 policy and rejection of other splits, module revocation, failed/reentrant launches, NFT movement during callbacks duplicate assets, separate self-mode accounting, no self-to-RF reward bypass, cross-mode reentry and merged fee recipients. Two Node tests cover explicit deployment configuration and the curated quote list. These tests do not constitute an audit.
...
This read-only script rejects missing/extra configuration fields, checks chain/module approvals/quote code and decimals at one block, checks that compiled bytecode matches the current source/settings, estimates contract-creation gas/current ETH cost, and writes an **unsigned** deployment payload plus a review manifest. The current creation estimate is 3,359,886 gas units; the initial reviewed price estimate was 0.000114074849472 ETH and must be refreshed by the signing wallet. It never loads a private key, signs or sends a transaction. The output refuses to overwrite an existing review. Mainnet preparation must be rerun if the code or chosen configuration changes.

Before enabling the app: review the successful real-module state-override evidence, implementation and exact immutable configuration, fund the approved wallet with Robinhood ETH, sign deployment explicitly in that wallet, verify source/bytecode and all immutable getters/quote entries, then set `RAREPET_LAUNCHPAD_ADDRESS` at build time. Configure server-side image storage separately. No frontend timer, local storage flag or preview action substitutes for the router’s cooldown or Brain ledger.
