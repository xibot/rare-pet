# RarePet development guide

[← Back to RarePet](../README.md)
...
- Genesis portraits use the same 36 canonical Generations bodies as Rare Rush. Change Body selects a different body and carries it into the embedded game; the original portrait stays intact. Six canonical Worlds and five Classic floors are selectable. Background islands always match the main island, with smaller flybys for Classic. Background visitors travel in three separate rows with paired visitors spaced half a circuit apart. They can pass behind the foreground pet and main island; they do not bounce off it. Widths scale down on narrow screens, and Classic visitors are smaller than Worlds visitors. Motion runs at 1.5× the original island speed. The island, chosen body and last preview Friend are remembered on this device. All showcase choices are cosmetic.
...
- Rare Launchpad at `/launch/` toggles Launch as Yourself and Launch as Your Rare Friend. Its form supports a local 512 × 512 image crop, token name/ticker, WETH or the full pinned Robinhood Chain stock/ETF token catalog with ticker/name search, and 0.3%/1%/2% trading fees. The owned-wallet flow signs an image-upload authorization, stores public image bytes under their content hash, embeds the URL/hash in onchain JSON metadata, reviews the exact token/pool/fees, then launches directly from the connected wallet or executes through the selected canonical Rare Wallet. Live launches and claims use the deployed Robinhood router listed below; each write still requires the creator’s wallet confirmation.
- Undeployed Solidity care ledger in `contracts/rare-pet`. Its implementation checks current ownership on every write. Each `(collection, tokenId)` has its own care state, designed to follow the NFT on transfer. Once deployed, this contract will enforce care cooldowns, rolling Play slots, decay, and rarity separately from original NFT metadata.
...
| Launch as Your Rare Friend | Once every 24 hours after a confirmed launch | +1 Brain in the separately deployed launch router ledger |
...
The care-model tests cover independent cooldowns, duplicate rewards, exact/overdue bond boundaries, projected decay, streaks, and rolling Play slots. Canonical body tests verify 324 frames, connected geometry, portrait placement and body selection. Browser checks exercise desktop and 390px/320px mobile layouts, both preview collections, isolated care/reset, island and body persistence, varied action effects, reduced motion, real game completion and no XP on incomplete games. Wallet fixtures check fresh ownership, network/account changes and switching back to Preview without signing. Contract tests additionally cover transfers, identity separation, signatures, replay, and fuzzed decay. Live-chain integration testing for the care contract is still outstanding. The launch router has separate recorded deployment, runtime and configuration checks; these are read-only verification and simulation, not a broadcast launch test. The automated suites do not prove that a real transfer, launch or fee claim has been broadcast successfully.

## Rare Wallet execution

Rare Wallet uses the FriendSDK 0.1.2 account interface from the SDK’s `src/chain.ts`: `execute(address,uint256,bytes,uint8)`, `owner()` and `token()`. Execution is restricted to CALL (operation 0), with zero outer ETH value. Supported calls are native ETH sends, ERC-20 `transfer`, and ERC-721/ERC-1155 `safeTransferFrom`. Ownership, canonical account binding, deployment, balances, network/provider identity and simulation are verified before asking the owner to sign. Receipt events and exact transaction calldata are checked before reporting confirmation.
...
Vercel Production and Preview configure the checked public address through `RAREPET_LAUNCHPAD_ADDRESS`. Image publication uses the existing server-only Blob credential. Anonymous visitors can still preview; connected creators review and sign their own image authorization, launch and fee-claim transactions. The local deployment helper is retained as review tooling, excluded from Vercel uploads, and stopped after the completed deployment.

These checks are not an independent security audit. Source is public, but explorer source publication is a separate step: the recorded Blockscout submission encountered an HTTP 403 challenge, so no explorer verification badge is claimed. See the [deployment manifest](../contracts/rare-launchpad/deployments/4663.json) and [launch router handoff](../contracts/rare-launchpad/README.md) for the receipt, exact checks, rehearsal limits and replacement-deployment procedure.
