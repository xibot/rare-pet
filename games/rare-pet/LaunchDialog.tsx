  ...
  const content = <>
    ...
    <div className="launch-content">
      ...
      {!review ? <form className="launch-form" onSubmit={reviewDraft}>
        ...
        <fieldset><legend>PAIR WITH</legend><div className="launch-options"><button type="button" aria-pressed={quoteId === 'weth'} onClick={() => setQuoteId('weth')}><b>Ξ WETH</b><small>Wrapped Ether</small></button><button type="button" aria-pressed={quoteId !== 'weth'} onClick={() => { if (quoteId === 'weth') setQuoteId('nvda'); }}><b>↗ STOCKS</b><small>Stock tokens</small></button></div>{quoteId !== 'weth' && <LaunchStockPicker value={quoteId} onChange={setQuoteId}/>}</fieldset>
