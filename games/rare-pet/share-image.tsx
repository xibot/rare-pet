...
function Reaction({ action, variant, size }: { action: ShareAction; variant: number; size: number }) {
  if (action === 'pet') {
    const color = ['#d46d7b', '#ab82c8', '#899f27'][variant];
    return <g>{[[-.06, .23, .12], [.93, .38, .085], [.82, .04, .12], [.1, .54, .075], [.38, -.11, .09]].map(([x, y, s], index) =>
      <Glyph key={index} x={size * x} y={size * y} size={size * s} color={color}><Heart/></Glyph>)}</g>;
  }
  if (action === 'feed') return <g>
    <Glyph x={size * .73} y={size * .53} size={size * .25}><Snack variant={variant}/></Glyph>
    ...
    {[0, 1, 2, 3].map(i => <rect key={i} x={size * (.71 + (i % 2) * .09)} y={size * (.76 + i * .055)} width={size * .025} height={size * .025} fill="#ac8153"/>)}
  </g>;
  const side = variant === 1 ? -.12 : .79;
  return <g>
    <svg x={size * (side - .11)} y={size * .7} width={size * .4} height={size * .3} viewBox="0 0 34 26" shapeRendering="crispEdges" opacity=".85">
    ...
    {[[-.05, .78, .08], [.19, .65, .06], [.24, 1.02, .06]].map(([x, y, s], index) => <Glyph key={index} x={size * (side + x)} y={size * y} size={size * s} color="#91ad31"><Spark/></Glyph>)}
  </g>;
}
...
async function drawSvg(context: CanvasRenderingContext2D, markup: string): Promise<void> {
  const url = URL.createObjectURL(new Blob([markup], { type: 'image/svg+xml;charset=utf-8' }));
  try {
    const image = new Image();
    await new Promise<void>((resolve, reject) => {
      const timer = window.setTimeout(() => reject(new Error('The share image took too long to load. Please try again.')), 15_000);
      image.onload = () => { clearTimeout(timer); resolve(); };
      image.onerror = () => { clearTimeout(timer); reject(new Error('Could not render this share image. Please try again.')); };
      image.src = url;
    });
...
export async function renderShareImage({ friend, island, bodyId, action, variant = 0 }: ShareImageOptions): Promise<Blob> {
  ...
  canvas.width = SHARE_IMAGE_SIZE; canvas.height = SHARE_IMAGE_SIZE;
  ...
  let markup: string;
  try {
    flushSync(() => root.render(<svg xmlns={SVG_NS} width={SHARE_IMAGE_SIZE} height={SHARE_IMAGE_SIZE} viewBox="0 0 2000 2000">
      ...
      <g transform={`translate(${offsetX} ${offsetY}) scale(${scale})`}>
        ...
        <g transform={`translate(${petLeft} ${petTop})`}>
          <g transform={pose(action, safeVariant, petSize)}><svg width={petSize} height={petSize} overflow="visible" data-share-slot="friend">
            {friend.collection === 'genesis' ? <GenesisPetSprite portraitUrl={friend.image} bodyId={bodyId} frame={safeVariant * 2}/>
              : <PetSprite sprites={friend.sprites!} frame={safeVariant * 2}/>}
          </svg></g>
          <Reaction action={action} variant={safeVariant} size={petSize}/>
        </g>
