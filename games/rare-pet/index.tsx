      ...
      <section className="pet-shell" aria-label="RarePet dashboard">
        ...
        <div className="care-layout">
          ...
          <div className={`habitat ${reaction ? `reaction-${reaction}` : ''}`}>
            ...
            <div className="friend-stage"><SpaceBackdrop island={island}/><div className="stage-coordinate">RF—{art?.tokenId ?? live?.tokenId ?? '000'}<br/>CARE. REPEAT. RARE.</div><div className="speech-bubble" key={reaction}>{reaction === 'pet' ? '♡ right back at you.' : reaction === 'feed' ? 'rare food. good mood.' : reaction === 'poop' ? 'ahh. much better.' : reaction === 'play' ? 'one run wiser. +10 XP!' : hasPet ? 'same time tomorrow?' : 'gm, new best friend.'}</div><FriendMotion action={reaction} sequence={reactionSequence} variant={reactionVariant}>{isGenesis ? <GenesisPetSprite portraitUrl={(art ?? live)!.image} bodyId={bodyId} frame={frame} walking={reaction === 'play'}/> : art?.collection === 'generations' ? <PetSprite sprites={art.sprites} frame={frame} walking={reaction === 'play'}/> : live?.sprites ? <PetSprite sprites={live.sprites} frame={frame} walking={reaction === 'play'}/> : <span className="missing-friend">?</span>}</FriendMotion><HabitatIsland island={island}/><span className="stage-mark left">+</span><span className="stage-mark right">+</span></div>
