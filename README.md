...
- Rare Launchpad at `/launch/` toggles Launch as Yourself and Launch as Your Rare Friend. Its form supports a local 512 × 512 image crop, token name/ticker, WETH or four verified stock pairs, and 0.3%/1%/2% trading fees. The owned-wallet flow signs an image-upload authorization, stores public image bytes under their content hash, embeds the URL/hash in onchain JSON metadata, reviews the exact token/pool/fees, then launches directly from the connected wallet or executes through the selected canonical Rare Wallet. Live launches and claims stay unavailable without the trusted router deployment.
...
Live launch activation is separate from live care: after review, deploy and verify the router, then build with `RAREPET_LAUNCHPAD_ADDRESS`. The frontend rechecks module addresses, supply, fee shares, quote allowance, NFT ownership and cooldown before asking for a transaction. Confirmations require an exact router event, expected token/pool and verified receipt block. Preview cannot publish, receive Brain or claim real fees. See [the launch contract handoff](contracts/rare-launchpad/README.md) for the unsigned deployment script and successful read-only full-route state-override verification. The standard Forge fork remains limited by public RPC access. No contract deployment or live launch transaction was sent during this implementation.
...
### Sign the reviewed router deployment

The public deployment review and configuration are in `contracts/rare-launchpad/`. The supplied treasury wallet is also the confirmed deployer. Both RF and self routes passed complete read-only mainnet simulations using constructor-generated runtime state overrides; no transaction was broadcast.

Run `node tools/launch-deploy/serve.mjs`, then open `http://localhost:4180` in a browser with your wallet extension. This local-only tool checks the review against the current source and compiled constructor data. It connects only on your click, refreshes the gas estimate, and sends only after you press Deploy and approve in your wallet. It persists pending deployment state and verifies the exact transaction and immutable configuration after mining. The wallet needs ETH on Robinhood Chain for gas.

Send the resulting router address for source verification and activation. The public `/launch/` remains a signless preview until `RAREPET_LAUNCHPAD_ADDRESS` is configured and the site rebuilt. The local deployment helper is excluded from Vercel uploads.
