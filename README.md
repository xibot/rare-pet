...
- Genesis portraits use the same 36 canonical Generations bodies as Rare Rush. Change Body selects a different body and carries it into the embedded game; the original portrait stays intact. Garden, Circuit, Crystal, Rooftop, Tidal and Rare islands, the chosen body and the last preview Friend are remembered on this device. All showcase choices are cosmetic.
...
Rare Friends artwork and FriendSDK are credited in the app and distributed `credits.txt`. Generations preview art and body frames use the cached public registry art from Rare Rush, with original provenance. Genesis preview portraits are verified original tokenURI SVGs cached at mainnet block 72489062 in `games/rare-pet/preview-genesis.json`; owned Genesis portraits are freshly validated. The body catalog and provenance are documented in `games/rare-rush/genesis/BODIES.md`. Fonts are the existing licensed Silkscreen, Archivo, and Sometype Mono assets. The main pet island, picker thumbnails and background flybys use the six official FriendSDK complete world presets also used in Rare Rush, preserving their original SVG artwork, palette and proportions. Saved choices from the older custom floors map to the corresponding canonical scenes. Action effects are decorative interface artwork, not replacement Rare Friends characters.
